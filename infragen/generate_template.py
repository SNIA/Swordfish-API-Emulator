...
def randStr(n):
    ...
    return ''.join([random.choice(alpanumeric) for _ in range(n)])
...
def read_properties(schema, root):
    ...
    if 'properties' in schema:
        ...
        return props
    if 'anyOf' in schema:
        pp(zip(range(len(schema['anyOf'])), schema['anyOf']))
